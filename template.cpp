...
vec3 abs3(const vec3& a) { return { fabsf(a.x),fabsf(a.y),fabsf(a.z) }; }

vec3 sgnv3(const vec3& a) { return vec3((float)((0.f) < a.x) - (a.x < (0.f)), (float)((0.f) < a.y) - (a.y < (0.f)), (float)((0.f) < a.z) - (a.z < (0.f))); }
...
int main( int argc, char **argv ) 
{
...
#ifdef ADVANCEDGL
#ifdef FULLSCREEN
	window = SDL_CreateWindow(TemplateVersion, 100, 200, ScreenWidth, ScreenHeight, SDL_WINDOW_FULLSCREEN|SDL_WINDOW_OPENGL );
#else
	window = SDL_CreateWindow(TemplateVersion, 100, 200, ScreenWidth, ScreenHeight, SDL_WINDOW_SHOWN|SDL_WINDOW_OPENGL );
#endif
...
#else
#ifdef FULLSCREEN
	window = SDL_CreateWindow(TemplateVersion, 100, 200, ScreenWidth, ScreenHeight, SDL_WINDOW_FULLSCREEN );
#else
	window = SDL_CreateWindow(TemplateVersion, 100, 200, ScreenWidth, ScreenHeight, SDL_WINDOW_SHOWN); //WIN64
#endif
	...
	while (!exitapp) 
	{
		...
		{
#if MOUSE_CONTROLS
			SDL_SetRelativeMouseMode(SDL_TRUE); //mouse wrap
#endif
			game->Init();
			firstframe = false;
		}
