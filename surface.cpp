...
void Surface::PrintNum(int number, int x1, int y1, Pixel color)
{
	char number_in_charP[100];
	if (!fontInitialized)
	{
		InitCharset();
		fontInitialized = true;
	...
	//const char* a_String = reinterpret_cast<char*>(&number);

	sprintf(number_in_charP, "%i", number);

	for (int i = 0; i < (int)(strlen(number_in_charP)); i++, t += 6)
	{
		long pos = 0;
		if ((number_in_charP[i] >= 'A') && (number_in_charP[i] <= 'Z')) pos = s_Transl[(unsigned short)(number_in_charP[i] - ('A' - 'a'))];
		else pos = s_Transl[(unsigned short)number_in_charP[i]];
		Pixel* a = t;
		char* c = (char*)s_Font[pos];
		for (int v = 0; v < 5; v++, c++, a += m_Pitch)
			for (int h = 0; h < 5; h++) if (*c++ == 'o') *(a + h) = color, * (a + h + m_Pitch) = 0;
